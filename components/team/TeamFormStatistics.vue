...
const {value: statistics, handleBlur: handleStatisticsBlur} = useField<Statistics>(
    "statistics",
    {},
...
<template>
  <BaseDragDrop :selected="selectedBanks" :unselected="unselectedBanks" @on-drop-selected="onDropSelected">
    <template #selectedLabel>
      <Translation keypath="team.banks-migration" tag="label" scope="global" class="custom-label">
        <u>{{ $t("common.to", 1) }}</u>
      </Translation>
    </template>
