...
const {teams} = await useTeamsFetch(ref<TeamQuery>({limit: 1, type: TeamType.EXAMPLE}));
...
const onBankEditClick = async (): Promise<void> => {
  if (confirm(t("common.save-lost-warning"))) {
    await navigateTo({
      name: "BanksEdit",
      params: {
        uuid: props.task.banks.find((bank) => bank.isActive && !bank.isAsync && bank.creditConfiguration)?.uuid ?? ""
      },
    });
  }
};
