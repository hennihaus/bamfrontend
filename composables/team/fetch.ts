...
export const useTeamsFetch = async (teamQuery?: Ref<TeamQuery>) => {
    ...
    return {
        ...
        pagination: computed(() => data.value?.pagination ?? {first: "", last: ""})
    }
}

export const useTeamFetch = async (uuid: Ref<string>, immediate: boolean = true) => {
    const {t} = useBaseI18n();

    const message = ref("");

    const {data: team} = await useBaseFetch<Team>(`${TEAMS_PATH}/${uuid.value}`, {
        watch: [uuid],
        immediate,
    });
    ...
    const resetStats = async (uuid: string): Promise<void> => {
        const {data} = await useBaseFetch<Team>(`${TEAMS_PATH}/${uuid}${STATISTICS_PATH}`, {
            method: "DELETE",
        });
        team.value = data.value;
        message.value = "";
