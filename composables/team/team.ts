...
export const useTeam = (team: Ref<Team>) => {
    ...
    const totalRequests = computed(() => {
        return Object.values(team.value.statistics)
            .reduce((requests, request) => requests + request, 0)
    });
    ...
    const statistics = computed<Statistic[]>(() => {
            return Object.entries(team.value.statistics).map(([bank, requests]) => ({
                    bank,
                    requests,
                    percentage: totalRequests.value ? requests / totalRequests.value : ZERO_REQUESTS,
