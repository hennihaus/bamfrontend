...
export const useBanksFetch = async (stopLoading: boolean = false) => {
    const {data: banks, refresh: refreshBanks} = await useBaseFetch<Bank[]>(BANKS_PATH, {stopLoading});

    const asyncActiveBanks = computed(() => banks.value?.filter(bank => bank.isAsync)?.filter(bank => bank.isActive));
...
export const useBankFetch = async (uuid: Ref<string>) => {
    ...
    const {data: bank} = await useBaseFetch<Bank>(`${BANKS_PATH}/${uuid.value}`, {
        watch: [uuid]
    });
    ...
    const deleteStatistics = async (bankId: string): Promise<void> => {
        await useBaseFetch<void>(`${STATISTICS_PATH}/${bankId}`, {method: "DELETE"});
